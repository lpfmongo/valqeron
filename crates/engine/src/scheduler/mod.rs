...
//!
//! `crate::scheduler` is the feature's only door: task modules and the engine
//! composition see [`Scheduler`] (the runtime handle),
//! [`SchedulerBuilder`] (registration), [`TaskDefinition`] with one
...
//!
//! Every task known to the code registers unconditionally; the registry
//! row is the source of truth for whether it runs (`enabled`) and for its
//! tunable settings. The boot reconcile seeds code defaults into NULL
//! settings columns, preserves operator values, and builds each trigger
//! from the *effective* (DB) settings.
//!
//! Inside, three runtime roles — types, not layers of files:
//!
//! - [`Scheduler`] / [`SchedulerBuilder`] — the **manager**:
...
//!   `enabled` gate, claiming, handler execution inside `task_run` spans,
...
use valqeron_core::{
    ...
    TaskKind, TaskRegistryRepository, TaskStatRepository, TaskStatusEntry, Versioned, WriteOutcome,
    list_task_statuses,
};

use crate::scheduler::task::{PendingTask, Registration};
use crate::scheduler::trigger::sync::ESCALATE_AFTER_FAILURES;
use crate::scheduler::trigger::{Interpretation, SeedPass, TaskFailure, TickMode};
...
const DISPATCH_MIN_SLEEP: Duration = Duration::from_millis(10);
...
impl Default for SchedulerBuilder {
    fn default() -> Self {
        Self {
            definitions: Vec::new(),
            dispatch_max_sleep: DISPATCH_MAX_SLEEP,
        }
    }
}
...
impl SchedulerBuilder {
    ...
    /// Override the dispatcher's sleep cap — tests seed rows directly
    /// (bypassing the seeders' notifications) and must not wait the
    /// production cap for the fallback sweep.
    #[cfg_attr(
        not(test),
        expect(dead_code, reason = "test knob; production keeps the default cap")
    )]
    pub fn dispatch_max_sleep(mut self, cap: Duration) -> Self {
        self.dispatch_max_sleep = cap;
        self
    }

    /// Run crash recovery and the catalog reconcile, then build each
    /// trigger from the registry's effective settings and start the
    /// workers. A kind registered twice is rejected with an error log — two
    /// triggers would race to seed it.
    pub async fn start(self, storage: AsyncStorage) -> Scheduler {
        let mut pending: Vec<PendingTask> = Vec::with_capacity(self.definitions.len());
        for definition in self.definitions {
            let kind = definition.kind;
            if pending.iter().any(|task| task.kind_str == kind) {
                tracing::error!(
                    kind,
            ...
            let Some(task) = definition.validate() else {
        ...
        recover_stale_running(&storage).await;
        let effective = reconcile_registry(&storage, pending).await;

        let mut registrations: HashMap<&'static str, Arc<Registration>> = HashMap::new();
        for task in effective {
            let registration = Registration {
                category: task.category,
                log_policy: task.log_policy,
                enabled: AtomicBool::new(task.enabled),
                handler: task.handler,
                trigger: trigger::build(task.kind_str, task.config),
            };
            registrations.insert(task.kind_str, Arc::new(registration));
        }
        ...
        let runner = Arc::new(TaskContextRunner {
            ...
            dispatch_max_sleep: self.dispatch_max_sleep,
        });

        let workers = TaskWorkerManager::launch(Arc::clone(&runner));
        Scheduler { runner, workers }
    }
}

// ================ THE FEATURE HANDLE ================
/// The running background-tasks feature: registration via
/// [`Scheduler::builder`], a bounded [`drain`](Self::drain) for
...
impl Scheduler {
    ...
    /// persisted `enabled` flag — and with the same semantics (an
    /// already-armed row still dispatches... until the claim skips the
    /// disabled kind). Returns whether a running seeder was stopped.
    #[cfg_attr(
        not(test),
        expect(
            dead_code,
            reason = "runtime task control; first RPC consumer is the enable/disable endpoint"
        )
    )]
    ...
    /// Wake a kind's seeder immediately (e.g. after flipping the enabled
    ...
    /// Flip a kind's `enabled` flag: **commit to the registry first**, then
    /// publish to the in-memory gate and wake the machinery (the seeder, so
    /// a re-enabled kind seeds immediately; the dispatcher, so frozen rows
    /// thaw). Memory is strictly a cache of committed state — a failed
    /// write publishes nothing. Returns whether a catalog row was updated.
    ///
    /// This is the single mutation path while the engine runs; raw-SQL
    /// edits of a live database are unsupported (stopped-engine only).
    #[cfg_attr(
        not(test),
        expect(
            dead_code,
            reason = "operator write path; first consumer is the enable/disable RPC"
        )
    )]
    pub async fn set_enabled(&self, kind: &str, enabled: bool) -> Result<bool, StorageError> {
        ...
        let applied = self
            .runner
            .storage
            .write("task_set_enabled", false, move |repos| {
                repos
                    .registry
                    .set_enabled(&task_kind, enabled, now)
        ...
        if applied {
            if let Some(registration) = self.runner.registrations.get(kind) {
                registration.enabled.store(enabled, Ordering::Relaxed);
                registration.trigger.wake();
            }
            self.runner.wake.notify_one();
            tracing::info!(
                target: "valqeron::audit",
                operation = "task_set_enabled",
                kind,
                enabled,
                "task enablement updated"
            );
        }
        Ok(applied)
    }
...
struct TaskContextRunner {
    ...
    /// fallback sweep.
    wake: Notify,
    /// The dispatcher's sleep watermark: the queue's earliest claimable
    /// `scheduled_at`, refreshed inside every empty claim. A *hint*, never
    /// authoritative — the rows themselves are the durable alarms, and the
    /// sleep cap self-heals a stale value.
    next_due: Mutex<Option<DateTime<Utc>>>,
    /// Upper bound on the dispatcher's sleep (production:
    /// [`DISPATCH_MAX_SLEEP`]; tests shrink it).
    dispatch_max_sleep: Duration,
}
...
async fn recover_stale_running(storage: &AsyncStorage) {
    let now = Utc::now();
...
/// The boot reconcile, one transaction: merge each task's stored settings
/// into its code config, upsert the declaration (descriptor computed from
/// the *effective* config; code defaults fill NULL settings only), retire
/// kinds that disappeared from code, and cancel their leftover pending
/// rows. Runs after crash recovery, so a retired kind's requeued rows are
/// cancelled too.
///
/// Returns the tasks with their effective configs. When the transaction
/// cannot run, the code defaults stand in — the engine boots either way.
async fn reconcile_registry(storage: &AsyncStorage, pending: Vec<PendingTask>) -> Vec<PendingTask> {
    let now = Utc::now();
    let fallback = pending.clone();
    let outcome = storage
        .write("task_registry_reconcile", false, move |repos| {
            let mut effective = Vec::with_capacity(pending.len());
            let mut kinds: Vec<TaskKind> = Vec::with_capacity(pending.len());
            for mut task in pending {
                let stored = repos.registry.get(&task.kind)?;
                if let Some(row) = &stored {
                    task.enabled = row.enabled();
                }
                let stored = stored.map(|row| *row.settings()).unwrap_or_default();
                task.config = task.config.with_settings(&stored);
                let declaration = TaskDeclaration {
                    kind: task.kind.clone(),
                    category: task.category,
                    trigger: task.config.trigger_kind(),
                    tracking: task.config.tracking(),
                    schedule: task.config.descriptor(),
                    source: task.config.source(),
                    log_policy: task.log_policy,
                    settings: task.defaults,
                };
                repos.registry.declare(&declaration, now)?;
                kinds.push(task.kind.clone());
                effective.push(task);
            }
            ...
            Ok::<_, StorageError>((effective, retired, cancelled))
        })
        .await;

    match outcome {
        Ok(Ok((effective, retired, cancelled_rows))) => {
            let retired_kinds: Vec<&str> = retired.iter().map(TaskKind::as_str).collect();
            tracing::info!(
                target: "valqeron::audit",
                operation = "task_registry_reconcile",
                declared = effective.len(),
                retired = retired.len(),
                cancelled_rows,
        ...
        Ok(Err(e)) => {
            tracing::warn!(error = %e, "task catalog reconcile failed; using code defaults");
            fallback
        }
        Err(e) => {
            tracing::warn!(error = %e, "task catalog reconcile not executed; using code defaults");
            fallback
        }
    }
}
...
impl TaskContextRunner {
    ...
    /// One durable seeding pass. The operator enable gate is the in-memory
    /// image of the committed registry flag — a disabled kind costs one
    /// atomic load and never touches the database. Writes to the flag go
    /// through [`Scheduler::set_enabled`] (DB first, publish after), so
    /// memory is never ahead of committed state.
    ///
    /// Returns the trigger's clock hint: when the seeder should re-pass
    /// ahead of its fallback tick (e.g. a cooldown expiry).
    async fn seed_pass(
        &self,
        kind: &'static str,
        registration: &Arc<Registration>,
    ) -> Option<DateTime<Utc>> {
        if !registration.enabled.load(Ordering::Relaxed) {
            tracing::debug!(kind, "task is disabled; seeding skipped");
            return None;
        }
        ...
        match pass {
            Ok(Ok(SeedPass::Seeded)) => {
                self.wake.notify_one();
                None
            }
            Ok(Ok(SeedPass::Idle { next_pass_at })) => next_pass_at,
            Ok(Err(e)) => {
                tracing::warn!(kind, error = %e, "task seeding pass failed");
                None
            }
            Err(e) => {
                tracing::warn!(kind, error = %e, "task seeding pass not executed");
                None
            }
        }
    }
    ...
    /// overlap. Deliberately not gated on `enabled`: ephemeral work is
    /// liveness work.
    ...
    /// How long the dispatcher may sleep: until the watermark's next due
    /// row, floored so a stale past watermark cannot busy-loop, capped so
    /// a wrong watermark self-heals within one fallback sweep.
    fn dispatch_sleep(&self) -> Duration {
        let watermark = self.next_due.lock().ok().and_then(|guard| *guard);
        match watermark {
            Some(at) => at
                .signed_duration_since(Utc::now())
                .to_std()
                .unwrap_or(Duration::ZERO)
                .min(self.dispatch_max_sleep)
                .max(DISPATCH_MIN_SLEEP),
            None => self.dispatch_max_sleep,
        }
    }
    ...
    /// budget. The final (empty) claim refreshes the sleep watermark in
    /// the same closure — retries and future schedules armed during this
    /// drain are all visible to that query.
        ...
        loop {
            ...
            let claimed = runner
                ...
                .write("task_claim", false, move |repos| {
                    let claimed = repos.tasks.claim_due(now, CLAIM_BATCH)?;
                    let next_due = if claimed.is_empty() {
                        repos.tasks.next_due_at()?
                    } else {
                        None
                    };
                    Ok::<_, StorageError>((claimed, next_due))
                })
                .await;

            let (claimed, next_due) = match claimed {
            ...
            if claimed.is_empty() {
                if let Ok(mut watermark) = runner.next_due.lock() {
                    *watermark = next_due;
                }
                return;
            }
...
/// The seeder's next pass instant: the trigger's clock hint when it is
/// sooner than the fallback cap, the cap otherwise. A past hint re-passes
/// immediately; a wall-clock hint is bounded by the cap, so clock jumps
/// self-heal.
fn next_pass_at(hint: Option<DateTime<Utc>>, cap: Duration) -> tokio::time::Instant {
...
impl TaskWorkerManager {
    ...
    /// One loop per registration: sleeps to the trigger's cadence (or its
    /// pass hint — a cooldown expiry sleeps exactly until then), wakes
    ...
    fn spawn_seeder(
        ...
        let handle = tokio::spawn(async move {
            let (first_pass, cap) = registration.trigger.cadence();
            let mut next_pass = first_pass;
            loop {
                tokio::select! {
                    _ = stopped.changed() => {
                ...
                let hint = match registration.trigger.mode() {
                    TickMode::Seed => runner.seed_pass(kind, &registration).await,
                    TickMode::Inline => {
                        runner.inline_run(kind, &registration).await;
                        None
                    }
                };
                next_pass = next_pass_at(hint, cap);
            }
        });
    ...
    fn spawn_dispatcher(
        ...
        tokio::spawn(async move {
            // Boot sweep: crash recovery may have requeued rows due now,
            // and the initial watermark must come from the real queue.
            TaskContextRunner::drain_due(&runner).await;
            loop {
                let pause = runner.dispatch_sleep();
                tokio::select! {
                    _ = shutdown.changed() => {
...
mod tests {
    ...
    use valqeron_core::{
        ...
        TaskCategory, TaskId, TaskSettings, TaskStats, TaskStatus, TaskTracking, TaskTrigger,
    };
    ...
    async fn failing_task_retries_until_it_succeeds() {
        ...
        // never interferes; the row is seeded directly (bypassing the
        // seeder's dispatcher notification, hence the tiny sweep cap).
        let manager = Scheduler::builder()
            .dispatch_max_sleep(Duration::from_millis(25))
            .task(
    ...
    /// The dispatcher is event-driven: with a huge sleep cap, a future row
    /// still dispatches at its own `scheduled_at` because the boot sweep's
    /// empty claim recorded it as the sleep watermark.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn future_row_dispatches_via_the_watermark_not_the_fallback() {
        let (_dir, storage) = storage();
        let due = Utc::now()
            .checked_add_signed(chrono::Duration::milliseconds(300))
            .expect("representable");
        seed_task(&storage, "test_watermark", Some(due), 1).await;

        let runs = Arc::new(AtomicUsize::new(0));
        let counter = Arc::clone(&runs);
        let manager = Scheduler::builder()
            .dispatch_max_sleep(Duration::from_secs(30))
            .task(
                interval("test_watermark", 600_000).run(move |_ctx: TaskContext| {
                    let counter = Arc::clone(&counter);
                    async move {
                        counter.fetch_add(1, Ordering::SeqCst);
                        TaskOutcome::Done
        ...
        // Well before the 30s cap: only the watermark can explain this.
        let probe = Arc::clone(&runs);
        wait_until(5, async || probe.load(Ordering::SeqCst) >= 1).await;
        assert!(manager.drain(Duration::from_secs(2)).await);
    }
    ...
    async fn past_due_recurring_row_from_a_previous_boot_runs_once() {
        ...
        let probe = Arc::clone(&runs);
        wait_until(5, async || probe.load(Ordering::SeqCst) >= 1).await;
        // Wait for the re-armed *future* row specifically: probing for any
        // queue row can match the still-RUNNING old one and the drain then
        // races the post-completion seed.
        let probe_storage = storage.clone();
        wait_until(5, async || {
            queued_tasks(&probe_storage).await.iter().any(|t| {
    ...
    async fn boot_reconcile_registers_the_catalog() {
        ...
        assert!(a.enabled(), "fresh rows are enabled");
        assert_eq!(
            a.settings().period_secs,
            Some(600),
            "code default seeded into the registry"
        );
        ...
        assert_eq!(c.settings().recurrence, Some(Recurrence::Daily));
        assert!(manager.drain(Duration::from_secs(2)).await);
    }

    /// A stored (operator-tuned) setting overrides the code default: the
    /// trigger is built from the DB value and the catalog descriptor shows
    /// the effective schedule.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn stored_settings_override_code_defaults_at_boot() {
        let (_dir, storage) = storage();

        // A previous boot cataloged the kind with its 600s code default;
        // an operator then tuned the period down to 1s.
        let declaration = TaskDeclaration {
            kind: TaskKind::new("test_tuned").unwrap(),
            category: TaskCategory::EngineSystem,
            trigger: TaskTrigger::Interval,
            tracking: TaskTracking::Durable,
            schedule: "interval:600s".into(),
            source: None,
        ...
        storage
            .write("test.tune", false, move |repos| {
                let now = Utc::now();
                repos.registry.declare(&declaration, now)?;
                let kind = TaskKind::new("test_tuned").map_err(|e| {
                    StorageError::Fault(valqeron_core::StorageFault::new(e.to_string()))
                })?;
                let tuned = TaskSettings {
                    period_secs: Some(1),
                    ..TaskSettings::default()
                };
                repos.registry.update_settings(&kind, &tuned, now)?;
                Ok::<_, StorageError>(())
            })
            .await
            .unwrap()
        ...
        let manager = Scheduler::builder()
            .task(
                interval("test_tuned", 600_000).run(move |_ctx: TaskContext| {
                    let counter = Arc::clone(&counter);
                    async move {
                        counter.fetch_add(1, Ordering::SeqCst);
                        TaskOutcome::Done
        ...
        // With the 600s code default the first run would be ten minutes
        // out; the 1s override must fire well within the deadline.
        let probe = Arc::clone(&runs);
        wait_until(10, async || probe.load(Ordering::SeqCst) >= 1).await;

        let row = registration_row(&storage, "test_tuned").await.unwrap();
        assert_eq!(
            row.schedule(),
            "interval:1s",
            "descriptor shows the effective schedule"
        );
        assert_eq!(row.settings().period_secs, Some(1), "override preserved");
        assert!(manager.drain(Duration::from_secs(2)).await);
    }
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn disabled_kind_seeds_nothing_until_reenabled() {
        let (_dir, storage) = storage();

        // Catalog the kind and disable it before the manager boots; the
        // boot reconcile must preserve the operator's flag.
        let declaration = TaskDeclaration {
            kind: TaskKind::new("test_disableable").unwrap(),
            category: TaskCategory::Other,
            trigger: TaskTrigger::Interval,
            tracking: TaskTracking::Durable,
        ...
        storage
            .write("test.disable", false, move |repos| {
                let now = Utc::now();
                repos.registry.declare(&declaration, now)?;
                let kind = TaskKind::new("test_disableable").map_err(|e| {
                    StorageError::Fault(valqeron_core::StorageFault::new(e.to_string()))
                })?;
        ...
        let manager = Scheduler::builder()
            .task(
                interval("test_disableable", 20).run(move |_ctx: TaskContext| {
                    let counter = Arc::clone(&counter);
                    async move {
                        counter.fetch_add(1, Ordering::SeqCst);
                        TaskOutcome::Done
        ...
        // Several ticks pass; the in-memory gate (captured from the
        // committed row at boot) must hold without touching the database.
        tokio::time::sleep(Duration::from_millis(300)).await;
        assert_eq!(runs.load(Ordering::SeqCst), 0, "disabled: nothing runs");
        assert!(queued_tasks(&storage).await.is_empty(), "nothing seeded");

        // Re-enable through the facade: committed first, published after,
        // and the seeder is woken — no fallback-tick wait.
        assert!(
            manager
                .set_enabled("test_disableable", true)
                .await
                .expect("facade write"),
        ...
        // Unknown kinds report no row rather than erroring.
        assert!(!manager.set_enabled("test_missing", false).await.unwrap());
        assert!(manager.drain(Duration::from_secs(2)).await);
    }

    /// Disabling a kind with an already-armed row freezes the row: the
    /// dispatcher's claim skips it, and re-enabling thaws it.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn armed_rows_of_a_disabled_kind_freeze_until_reenabled() {
        let (_dir, storage) = storage();

        let declaration = TaskDeclaration {
            kind: TaskKind::new("test_frozen").unwrap(),
            category: TaskCategory::Other,
            trigger: TaskTrigger::Interval,
            tracking: TaskTracking::Durable,
            schedule: "interval:600s".into(),
        ...
        storage
            .write("test.freeze", false, move |repos| {
                let now = Utc::now();
                repos.registry.declare(&declaration, now)?;
                let kind = TaskKind::new("test_frozen").map_err(|e| {
                    StorageError::Fault(valqeron_core::StorageFault::new(e.to_string()))
                })?;
                repos.registry.set_enabled(&kind, false, now)?;
                Ok::<_, StorageError>(())
        ...
        let counter = Arc::clone(&runs);
        let manager = Scheduler::builder()
            // A tiny sweep cap keeps the dispatcher claiming throughout the
            // frozen phase: the row survives because the claim's SQL filter
            // skips disabled kinds, not because nothing looked.
            .dispatch_max_sleep(Duration::from_millis(25))
            .task(
                // A long period: the seeder never fires within the test;
                // only the directly seeded row matters.
                interval("test_frozen", 600_000).run(move |_ctx: TaskContext| {
                    let counter = Arc::clone(&counter);
                    async move {
                        counter.fetch_add(1, Ordering::SeqCst);
                        TaskOutcome::Done
        ...
        // A due row exists, but the kind is disabled: the claim skips it.
        seed_task(&storage, "test_frozen", None, 1).await;
        tokio::time::sleep(Duration::from_millis(300)).await;
        assert_eq!(runs.load(Ordering::SeqCst), 0, "frozen: the claim skips");
        assert_eq!(queued_tasks(&storage).await.len(), 1, "the row waits");

        // Thaw through the facade; the dispatcher is notified directly.
        assert!(manager.set_enabled("test_frozen", true).await.unwrap());
